...
import com.example.unchain.data.defaultAddictions.DefaultTestValues
...
class InitDbUseCase @Inject constructor(private val addictionDao: AddictionDao, private val mapper: Mapper, private val personalizationRepositoryImpl: PersonalizationRepositoryImpl) {
    suspend fun execute(){
        addictionDao.insertAll(mapper.entityToDbModelAddictionList(DefaultTestValues.list))
        for (i in DefaultTestValues.listPersonality){
